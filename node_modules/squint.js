...
var commentSingle = new RegExp( '\/\/.*\n?', 'g' )
  ...
  , typeForward = new RegExp( 'class[^\{]*;|template[^\{]*;|struct[^\{]*;' )
  , functionForward = new RegExp( '.*?\\\)[ \t]*?;' )
  , subScopes = new RegExp( '{.*?}', 'g' );
  
module.exports.stripSubScopes = function( code ) {
  code = code.replace( subScopes, '' ); 
  return code;
};
  
module.exports.getSubScopes = function( code ) {
  code = code.match( subScopes ); 
  return code;
};
...
module.exports.getTypeForwardDeclares = function( code ) {
  code = code.match( typeForward );
  if (!code) {
    code = '';
  }
  return code;
