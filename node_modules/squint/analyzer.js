...
function Analyzer() {
  ...
  this.on( 'open', function( code ) {
    if (code.search( namespaceDeclare ) != -1) {
      var name = code.replace( namespaceDeclarator, '' );
      this.emit( 'open namespace', name );
    }
    else if (code[ code.length - 1 ] == ')')
    {
      if (code.search( templateHeader ) == 0) {
        this.emit( 'open template function', name );
      } 
      else {
        this.emit( 'open function', name ); 
      }
    }
    else if (code.search( templateHeader ) == 0) {
      this.emit( 'open template type', name );
    }
    else {
      this.emit( 'open type', name );
    }
  } );
}
